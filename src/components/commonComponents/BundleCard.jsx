...
const BundleCard = ({ course }) => {
  const { purchasedBundles } = useSelector((state) => state.user);
    ...
    <>
      {course && (
        <div className="course-item">
          <Link to={`/bundle/${course?.slug}`}>
            <figure className="mb-0">
              <img
                src={`${process.env.REACT_APP_BACKEND_URL}/uploads/bundle/thumbnails/${course?.thumbnailImage}`}
                alt=""
          ...
          <div className="course-more-details">
            ...
            <div className="more_btn_wrap">
              <div className="more-details-action-wrap">
                {/* TODO: */}
                {!purchasedBundles?.includes(course?._id.toString()) ? (
                  <>
                    {cart?.find(
                ...
                <Link to={`/bundle/${course?.slug}`} className="redirect-link">
                  View course
                </Link>
              </div>
            </div>
